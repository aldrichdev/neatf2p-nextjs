...
const RenameAccountModalBody = (props: RenameAccountModalBodyProps) => {
    ...
    return (
      <>
        ...
        <p>
          ...
          this,{' '}
          <strong>
            <span className='text-red-600'>you can never go back to the account name you have now</span>
          </strong>
          , so please proceed carefully.
        </p>
        <Button onClick={handleRestore} disabled={restoreButtonDisabled}>
          Restore Username
        </Button>
        {restoreSuccessMessage && (
  ...
  return (
    <>
      ...
      <span className='text-red-500'>You can only rename an account once</span>, so please choose wisely when picking a
      new account name. Any spaces at the beginning or end of your new name will not be counted, but spaces within are
      fine.
    </>
  )
}
